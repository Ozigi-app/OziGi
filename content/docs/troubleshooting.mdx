...
2. **The ICP is too vague.** The opposite failure: queries so broad that nothing scores above the 0.3 cut-off.
3. **They are sourcing, then being discarded.** Leads scoring under 0.3 are dropped as they are found. The cut-off is fixed, so the fix is a sharper ICP rather than a setting — add concrete criteria and explicit exclusions.
...
5. **Did leads clear the 0.3 score cut-off?** Anything under it was discarded during sourcing and never reached the campaign.
...
Expected — the connection request *is* the LinkedIn channel. Ozigi does not send follow-up DMs after someone accepts, and does not message Open Profile leads. Once the request goes out, that lead is done on LinkedIn.

To follow up, reach the lead on email, where a sequence can run as many steps as you want.
