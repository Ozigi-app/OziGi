...
export async function loadSession(session: SessionInfo): Promise<{ context: BrowserContext }> {
  ...
  // NOTE: a resource-block route (aborting image/media/font) was tried here to speed
  // up profile loads on the slow proxy, but it correlated with degraded profile
  // renders — the top-card action row (Message button, degree badge) failing to
  // appear and BLANK_PAGE errors. Reverted: we now let all resources load so the
  // SPA hydrates the full profile the same way a real browser does.
